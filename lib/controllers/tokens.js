...
_tokens.DELETE = (req, callback) => {
  ...
  _data.read(DB_DOCS.TOKENS, token, function (err, data) {
    if (err || !data)
      return callback(400, errorResponse("could not find token"));

    _data.delete(DB_DOCS.TOKENS, token, function (err) {
      if (err) return callback(400, errorResponse("could not delete token"));
      return callback(201);
    });
  });
};
